...
def content_loss(content_weight, content_current, content_target):
    ...
    content_current = content_current.squeeze(0).view((C, -1))
    content_target = content_target.squeeze(0).view((C, -1))

    return content_weight*torch.norm(torch.add(content_current,-content_target))**2
...
def gram_matrix(features, normalize=True):
    ...
    for i in range(N):
        gram_matrix.append(torch.matmul(features[i], features_transpose[i]))

    
    gram_matrix = torch.stack(gram_matrix)
    if normalize:
        return torch.div(gram_matrix, H*W*C)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer in enumerate(style_layers):
        current = gram_matrix(feats[layer].clone()).squeeze(0)
        target = style_targets[i].squeeze(0)
        style_loss += style_weights[i]*torch.norm(torch.add(current,-target))**2
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    height_top = img.squeeze(0)[:, :-1,:]
    height_bottom = img.squeeze(0)[:, 1:, :]
    width_top = img.squeeze(0)[:, :,:-1]
    width_bottom = img.squeeze(0)[:, :, 1:]

    return tv_weight*(torch.norm(torch.add(height_top, -height_bottom))**2+torch.norm(torch.add(width_top, -width_bottom))**2)
