...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    model = nn.Sequential(
    nn.Linear(noise_dim, input_size),
    nn.ReLU(),
...
def discriminator_loss(logits_real, logits_fake):
    ...
    devi = torch.cuda.FloatTensor
    labels = torch.ones(2*size).type(devi)

    labels = torch.cat((torch.ones(size), torch.zeros(size))).type(devi)
    # print(labels.shape)
    scores = torch.cat((logits_real, logits_fake))
    # print(scores.squeeze(1).shape)
    loss = 2*bce_loss(scores.squeeze(1), labels)
...
def generator_loss(logits_fake):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    size = logits_fake.shape[0]
    labels = torch.ones(size).type(torch.cuda.FloatTensor)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5*torch.mean(scores_fake**2)+0.5*torch.mean((scores_real-1)**2)
